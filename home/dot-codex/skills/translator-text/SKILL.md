...
이 스킬은 **마크다운이 아닌 평문 텍스트 파일 한 개**를 다른 언어로 번역해 새 파일로 쓰고, 현재 Codex 세션으로는 **3줄 요약만** 리턴한다. 마크다운 의미 분석은 하지 않는다 — 줄 단위 구조와 식별자 토큰(URL, 파일 경로, 자막 타임스탬프 등)만 보존한 채 산문을 번역한다.
...
2. target 경로 결정 (없을 때) — 모든 경로 조작은 모델이 직접 문자열로 계산한다. `shell` 호출이나 shell parameter expansion(`${VAR%...}`, `${VAR##*.}`, `basename`, `dirname` 등) 사용 금지.
...
- 보고: 번역 본문 전체·언어 감지 비율·라인별 매핑 같은 중간 산출물은 출력하지 않는다.
