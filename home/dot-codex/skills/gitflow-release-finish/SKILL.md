...
4. 마무리 규칙을 참고하여 리턴 형식대로 출력 후 스킬 실행을 마친다.
...
- 보고: 검증 로그·차이 요약은 헤딩 위쪽에 두거나 출력 안 함. 최종 응답에 포함하지 않는다.
