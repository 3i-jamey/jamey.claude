...
5. 마무리 규칙을 참고하여 리턴 형식대로 출력 후 스킬 실행을 마친다.
...
- 보고: 현재 버전 추출 과정·검증 로그는 헤딩 위쪽에 두거나 출력 안 함. 최종 응답에 포함하지 않는다.
...
진행 응답을 받으면 Codex 세션이 `git flow release start <VERSION>` → 버전 파일 수정 → `git add` + 커밋 → `git push -u origin release/<VERSION>` 순으로 실행. 새 브랜치명·갱신된 버전·커밋 SHA 보고.
