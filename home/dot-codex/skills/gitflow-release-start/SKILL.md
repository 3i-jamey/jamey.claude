...
3. 새 버전 결정:
   - `사용자 요청`가 있으면 `v` 접두사 제거 후 `MAJOR.MINOR.PATCH` 검증.
...
5. 마무리 규칙을 참고하여 리턴 형식대로 출력 후 스킬 실행 종료.
...
- 보고: 현재 버전 추출 과정·검증 로그는 출력하지 않는다.
...
진행 응답을 받으면 현재 Codex 세션이 `git flow release start <VERSION>` → 버전 파일 수정 → `git add` + 커밋 → `git push -u origin release/<VERSION>` 순으로 실행. 새 브랜치명·갱신된 버전·커밋 SHA 보고.
