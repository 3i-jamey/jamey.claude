...
1. **입력 모드 판별**: `사용자 요청`가 브랜치명·PR번호·파일 경로 중 무엇인지 판단:
...
- `AGENTS.md` / `CONTRIBUTING.md` 명시 규칙 위반은 항상 **🚨 Critical**.
- 한국어로 작성한다.
...
- 보고: 변경 파일 본문·blame raw·grep 결과·중간 추론은 출력하지 않는다.
...
진행 응답을 받으면 현재 Codex 세션이 PR이 있을 때:
