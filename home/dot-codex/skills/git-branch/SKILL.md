...
7. **이슈 트래커 자동 조회**: `Jira/Atlassian issue 조회 도구`로 티켓 정보(summary, issuetype)를 가져와 슬러그·prefix 결정에 활용한다. 실패하면 묻는다.
8. 최종 브랜치 이름 조합:
   ```
   feature/<TICKET>-<slug>
   bugfix/<TICKET>-<slug>
...
- 보고: 슬러그 후보·티켓 raw·기존 브랜치 검색 결과는 출력하지 않는다.
...
진행 응답을 받으면 현재 Codex 세션이 `git fetch origin && git checkout -b <브랜치명> --no-track origin/<베이스>` 실행. upstream 비어 있는지 `git branch -vv`로 확인 후 SHA와 브랜치명 보고.
