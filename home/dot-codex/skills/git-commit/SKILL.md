...
6. **타입 힌트 수집**: 브랜치명에서 티켓(`[A-Z]+-\d+`)을 추출한다. Jira URL이나 프로젝트 설정에서 `cloudId`를 알 수 있으면 `mcp__jira__getJiraIssue`로 이슈 타입(Bug/Story/Task)을 조회해 commit 타입 결정에 참고한다. `cloudId`가 없으면 `mcp__jira__getAccessibleAtlassianResources`로 확인하고, 실패하거나 모호하면 Jira 조회 없이 diff만 보고 타입을 판단한다.
...
- 보고: diff 본문·MCP raw 응답·중간 추론은 헤딩 위쪽에 두거나 출력 안 함. 최종 응답에 포함하지 않는다.
...
진행 응답을 받으면 Codex 세션이 위 메시지 그대로 HEREDOC으로 `git commit` 실행. pre-commit 훅 실패 시 근본 원인 보고 후 멈춘다 (`--amend` 금지).
