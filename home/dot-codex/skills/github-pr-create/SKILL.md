...
5. PR 초안 작성:
   ...
   - **본문** 구조 (HEREDOC 본문 그대로 사용):
     ...
     ```
6. 베이스 브랜치 결정:
   - `사용자 요청`에 베이스가 명시(`master`, `main`, `release/X.Y.Z`)되면 그것 사용
   - 그 외는 **`stage`** 기본값
   - hotfix 브랜치라도 별도 지시 없으면 stage가 안전한 기본값
7. 마무리 규칙을 참고하여 리턴 형식대로 출력 후 스킬 실행 종료.
...
진행 응답을 받으면 현재 Codex 세션이 업스트림 없는 브랜치면 `git push -u origin <브랜치>` 실행 후, HEREDOC 본문으로 `gh pr create --base <베이스> --title <제목>` 실행. 생성된 PR URL 보고.
