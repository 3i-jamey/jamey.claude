...
1. 인자 파싱: `사용자 요청`에서 `source`(필수), `target`(선택), `target-lang`(선택)을 토큰으로 분리한다. `source`가 없으면 `BLOCKED: NO_INPUT`로 종료.
2. `shell`로 source 존재·종류 확인:
...
6. 관련 Codex skill로 결정된 워커를 호출하고 원래 받은 `사용자 요청`를 그대로 전달한다.
7. 워커가 리턴한 4줄(헤더 1 + 불릿 3)을 가공 없이 그대로 노출하고 종료. 추가 헤더·요약·주석을 붙이지 않는다.

## 강제 규칙(!) : !는 IMPORTANT와 동일

- 자체적으로 파일을 읽거나 쓰지 않는다 — 파일 읽기/파일 쓰기 권한 없음.
- 자체 번역하지 않는다.
...
- 결과: 워커의 리턴 형식이 그대로 현재 Codex 세션에 노출된다. 라우터는 추가 출력하지 않는다.
