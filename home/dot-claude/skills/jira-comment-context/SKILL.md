...
model: 'sonnet'
context: 'inherit'
tools: 'Skill, mcp__claude_ai_Atlassian_Rovo__getJiraIssue, mcp__claude_ai_Atlassian_Rovo__addCommentToJiraIssue'
---
...
2. **이슈 확인**: `mcp__claude_ai_Atlassian_Rovo__getJiraIssue`로 이슈를 조회해 `cloudId`를 확보한다. (MCP 서버 이름이 `_Rovo_` 없는 변종으로 노출되면 그것을 사용 — 둘 다 같은 Atlassian MCP)
3. **컨텍스트 분석**: `Skill` 도구로 `analyzer-context`를 호출한다 (인자 없음 — 현재 대화를 분석).
...
진행 응답을 받으면 ADF 골격(영어 heading + 요약 + expand[내용 ADF] → 한글 heading + 요약 + expand)을 조립해 `mcp__claude_ai_Atlassian_Rovo__addCommentToJiraIssue`(contentFormat: "adf", cloudId는 getJiraIssue 응답에서) 호출. 등록된 댓글 URL 보고.
