...
자유 문구에서 핵심 키워드만 추출해 3~4단어 이내의 짧은 kebab-case 식별자를 만든다. git 브랜치명·폴더명으로 그대로 쓰일 수 있도록 짧고, 가독성이 있어야 한다.

## 워크플로우

1. 입력 문자열을 받는다(없으면 호출자에게 다시 요청).
...
5. 결과 슬러그 한 줄만 출력한다(설명·코드펜스 없이).

## 반드시 지킬 규칙

- 출력은 **3~4단어 이내**로 제한한다 — git 브랜치명으로 그대로 쓰이므로 길면 안 된다.
- 출력은 `[a-z0-9-]+` 만 포함한다 — 식별자로 안전해야 한다.
- 슬러그 한 줄만 출력한다 — 설명·따옴표·코드펜스를 붙이지 않는다(호출자가 그대로 사용).
- 의미가 사라질 정도로 줄이지 않는다 — 주체/현상이 빠지면 식별 불가.

## 기본 규칙

- 원문이 이미 짧고 의미가 명확하면 그대로 kebab-case로만 변환한다 — 억지로 더 줄이지 않는다.
- 의미를 해치지 않는 선에서 합성어는 한 단어로 붙인다(예: "Auto Zoom" → `autozoom`) — 단어 수를 아낀다.
- 동의어 중 짧은 쪽을 고른다(예: "vibration and jittering" → `jitter`) — 토큰 절약.
- 4단어로도 모호하면 가장 식별력 있는 명사 1개를 남기고 수식어를 버린다 — 길이 우선.
