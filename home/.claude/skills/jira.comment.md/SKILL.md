...
3. **언어 자동 판별**: 각 파일별로 한글 음절(U+AC00–U+D7AF) 비율과 ASCII 알파벳 비율을 비교한다.
   ...
   - 두 파일이 같은 언어로 분류되거나 비율이 모호하면 임의 결정하지 말고 사용자에게 어느 쪽이 어떤 언어인지 묻는다.
4. **이슈 확인**: `mcp__claude_ai_Atlassian_Rovo__getJiraIssue`로 이슈를 조회해 `cloudId`를 확보한다.
...
7. **ADF 조립**: 단일 댓글 문서를 다음 골격으로 만든다.
   ...
   ```
8. **댓글 등록**: `mcp__claude_ai_Atlassian_Rovo__addCommentToJiraIssue` 호출.
   - `contentFormat: "adf"` 필수.
   - `cloudId`는 `getJiraIssue` 응답에서 가져온 값 사용.
9. **완료 보고**: 등록된 댓글 URL과 두 파일의 언어 분류 결과(어느 파일이 영어/한글로 인식됐는지)를 보고한다.

## 반드시 지킬 규칙

- 본문(두 마크다운 파일 내용)은 반드시 `expand`로 감싼다 — 댓글이 길어져 이슈 화면을 잡아먹지 않게 한다.
- `expand.attrs.title`에는 국기 이모지를 넣지 않는다 — 이모지는 상단 heading에만 사용. 펼친 화면이 이모지로 시끄러워지지 않게 한다.
- `contentFormat: "adf"`를 사용한다 — markdown 포맷에는 `expand` 노드가 존재하지 않는다.
- 한 줄 요약은 한 줄(80자 내외)을 넘기지 않는다 — 길어지면 expand의 의미가 사라진다.
- 언어 판별이 모호하면 임의로 결정하지 말고 사용자에게 묻는다 — 영어/한글이 뒤바뀌면 글로벌 독자에게 잘못 노출된다.
- 파일이 없거나 비어 있으면 댓글을 달지 않는다 — 빈 expand는 노이즈만 만든다.
- `cloudId`는 하드코딩 금지 — `getJiraIssue` 응답에서 추출한다.

## 기본 규칙

- 두 블록은 항상 영어 → 한글 순서로 둔다 — 글로벌 가독성 우선.
- 마크다운 코드 펜스의 언어 힌트(```` ```ts ````)는 ADF `codeBlock.attrs.language`로 보존한다.
- 마크다운 표(`|`)는 가능하면 ADF `table`로 변환하고, 구조가 복잡하면 `codeBlock`으로 폴백한다.
- 댓글은 한 번에 하나만 등록한다 — 두 파일을 단일 댓글로 묶는다.
...
- 입력: `https://acme.atlassian.net/browse/PROJ-12 ./en.md ./ko.md`
  동작: URL에서 `PROJ-12` 추출 → 동일 절차로 댓글 등록
