...
4. **PR 찾기:**
   ```bash
   gh pr list --head <current-branch> --json number --jq '.[0].number'
   ```
   - PR 있음 → step 5
   - PR 없음 → 출력 포맷대로 터미널 출력 후 종료

5. **PR 제출** — 두 단계:

   **5-1. 라인 코멘트** (각 이슈마다):
   ```bash
   OWNER_REPO=$(gh pr view <PR> --json repository --jq '.repository.nameWithOwner')
   COMMIT=$(gh pr view <PR> --json headRefOid --jq '.headRefOid')

   gh api repos/$OWNER_REPO/pulls/<PR>/comments \
     -f body="🚨 **[크래시 위험] NPE 가능성**

   \`user!!.name\` 사용. user가 null일 경우 크래시.

   **제안**: \`user?.name ?: \"Unknown\"\`" \
     -f path="app/src/main/java/.../UserViewModel.kt" \
     -f commit_id="$COMMIT" \
     -F position=10
   ```
   - `path`: repository root 기준 상대 경로
   - `position`: **diff 상의 라인 번호** (파일 절대 라인 ≠). `git diff stage...HEAD --unified=0 <file>`에서 계산
   - `commit_id`: PR head SHA

   **5-2. 종합 review 제출** (verdict 기반):
   ```bash
   gh pr review <PR> --request-changes -b "<summary>"   # Critical 있음
   gh pr review <PR> --comment         -b "<summary>"   # Major 있음
   gh pr review <PR> --approve         -b "<summary>"   # 이슈 없음
   ```

## 반드시 지킬 규칙

- 코드를 수정·생성·삭제하지 않는다 — 리뷰만 수행한다.
- 저장소 상태를 변경하는 git 명령을 실행하지 않는다 (commit/push/merge/rebase/reset 등 금지).
- 허용되는 git 명령만 사용한다: `fetch`, `pull`, `checkout`, `branch`, `log`, `diff`, `show`, `status` — 그 외는 사용 금지.
- 추측이 아닌 **실제 코드를 인용**해서 리뷰한다 — 코드 근거 없는 지적은 거짓 양성을 만들고 신뢰를 떨어뜨림.
- 모든 이슈에 **파일 경로 + 라인 번호**를 포함한다 — 추적 불가능한 지적은 무가치함.
- `CLAUDE.md` / `CONTRIBUTING.md` 명시 규칙 위반은 항상 **🚨 Critical** — 팀 합의가 개인 취향보다 우선.
- 한국어로 작성한다.
- 변경이 없거나 이슈가 없으면 그대로 보고하고 이슈를 지어내지 않는다 — 거짓 양성은 다음 리뷰가 무시되는 원인.

## 기본 규칙

- 잘된 점을 **최소 1개** 언급한다 — 지적만 있으면 팀 분위기 해침. 없으면 "리뷰 통과" 수준이라도 명시.
- 가능하면 참고 자료 링크를 첨부한다 (Android 공식, Kotlin 문서, 베스트 프랙티스) — 리뷰가 학습 자료가 되도록.
- 리뷰 제출 타입은 자동 결정: Critical → request-changes / Major 있음 → comment / 이슈 없음 → approve.
- 변경 파일 10개+ 시 사용자에게 "대규모 변경이라 시간 걸립니다" 한 번 알린다.
- 단일 파일 diff 300줄+ 시 섹션별로 나눠서 리뷰 — 한 번에 다 읽으면 정확도 떨어짐.
- 이슈가 없으면 "이상 없음" 명시한다 — 침묵보다 명시가 신뢰를 줌.
